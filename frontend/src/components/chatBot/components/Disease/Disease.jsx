    ...
    return filtered_list.length !== 0 ? (
      <div className="grid-row width-full DiseaseComponent">
        <div className="col-12 tablet:grid-col-12 patientInfo">
          <h3 style={{color: "white"}}>Patient gender: {this.props.gender}</h3>
          <h3 style={{color: "white"}}>Patient age: {this.props.age}</h3>
        </div>
        <div className="col-12 tablet:grid-col-12 patientQuestions">
          {this.state.patientInfo.map((key, id) => (
            <div className="singleQuestion" key={id}>
              <p style={{color: "#A1927C "}}>{key.question}</p>
              <p style={{color: "white"}}>{key.answer}</p>
            </div>
          ))}
        </div>
        <div className="col-12 tablet:grid-col-12 DiagnosisReport">
          <h2 style={{color: "white"}}>Diagnosis Report</h2>
          {filtered_list.map((key, id) => (
            <div className="reportDiv" key={id}>
              ...
              <div className="diseaseSymptoms">
                <h4 style={{color: "white"}}>Disease Symptoms</h4>
                <ul>
                  {key.disease_symptom.sort().map((item, index) => {
                    return key.symptom_user_has.includes(item) ? (
                      <li style={{color: "#A1927C"}} key={index} className="active">
                        {item}
                      </li>
                    ) : (
        ...
        <div style={{color: "red"}}>Always visit a doctor if you have any symptoms of a disease or call your local hospital</div>
      </div>
    ) : (
      <React.Fragment>
        <div className="grid-row width-full DiseaseComponent">
          ...
          <p style={{color: "greenyellow"}}> Cannot determine possible diseases due to lack of symptoms. Please retry the analysis with actual symptoms or call your local hospital if it is an emergency.</p>
        </div>
      </React.Fragment>
    );
  };
